...
const ProjectCard = ({ project, index }) => {
  return (
    <motion.div
      ...
      className="group relative bg-slate-900/40 border border-white/5 rounded-2xl p-6 hover:border-blue-500/30 hover:bg-slate-900/60 transition-all duration-300 shadow-lg hover:shadow-blue-500/10"
    >
      {/* Background Glow */}
      <div className="absolute inset-0 bg-blue-500/5 opacity-0 group-hover:opacity-100 transition-opacity blur-3xl -z-10" />

      <div className="flex flex-col h-full">
        <div className="relative mb-4 overflow-hidden rounded-xl bg-slate-800 aspect-video">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech.map((t) => (
            <motion.span 
              ...
              className="text-[10px] font-mono uppercase tracking-wider px-2 py-1 bg-blue-500/5 border border-white/5 rounded-md text-slate-300"
            >
              {t}
            </motion.span>
          ))}
